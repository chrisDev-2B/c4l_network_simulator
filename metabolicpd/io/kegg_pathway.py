...
def network_from_KGML(kegg_file_path):
    """ Loads a network from a kegg file path. If file isn't found, fetches from Kegg's REST API

    Args:
        kegg_file_path (Path): Path to the kegg file within data/kegg/

    Returns:
        res_path (Path): Path to the kegg file
    """
    out_dir = os.path.dirname(kegg_file_path)
    kegg_id = os.path.split(kegg_file_path)[-1]  # the tail of the file path is the kegg id
